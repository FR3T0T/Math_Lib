...
classdef MatIngBib
    methods(Static)
        ...
        function koeff = taylorPoly(func, x0, n)
            ...
            % Returnerer en vektor med koefficienter [a_0, a_1, a_2, ..., a_n]
            ...
            try
                ...
                % Numerisk tilnaermelse hvis symbolsk metode fejler
                koeff = zeros(1, n+1);
                h = 1e-8;
                
                % Beregn funktion og numeriske afledte
                koeff(1) = func(x0);  % 0'te afledte = funktionsvaerdi
                
                % For hoejere ordens afledte
                for i = 1:n
                    % Implementer en mere praecis numerisk differentiationsmetode
                    if i == 1
                        % Centreret differenskvotient for foerste afledte
                        koeff(i+1) = (func(x0 + h) - func(x0 - h)) / (2 * h);
                    else
                        % Brug finitte differenser for hoejere ordens afledte
                        % Dette er en simpel implementering; mere sofistikerede metoder kan bruges
                        df = zeros(1, 2*i+1);
                        x_vals = x0 + (-i:i) * h;
