function [F_t, forklaringsOutput] = fourierTidMultiplikation_med_forklaring(f, t, omega, n, kompleks)
    ...
    % Start forklaring
    if kompleks
        forklaringsOutput = ElektroMatBibTrinvis.startForklaring(['Fouriertransformation af (-jt)^' num2str(n) '·f(t)']);
    else
        forklaringsOutput = ElektroMatBibTrinvis.startForklaring(['Fouriertransformation af t^' num2str(n) '·f(t)']);
    end
    ...
    for i = 1:n
        if i == 1
            F_t = 1i * diff(F, omega);
            forklaringsOutput = ElektroMatBibTrinvis.tilfoejTrin(forklaringsOutput, 3+i, ...
    ...
    if kompleks
        forklaringsOutput = ElektroMatBibTrinvis.tilfoejTrin(forklaringsOutput, 4+n, ...
