function [F, forklaringsOutput] = enhedsTrin_med_forklaring(t0, t, s)
    ...
    % Håndter både symbolske og numeriske værdier af t0
    if isa(t0, 'sym')
        % For symbolske variable
        % Håndter som en generel symbolsk parameter med betingelser
        forklaringsOutput = ElektroMat.Forklaringssystem.tilfoejTrin(forklaringsOutput, 4, ...
            'Generel løsning for symbolsk værdi', ...
            ['For et symbolsk t0 = ' t0_str ' kan vi udlede den generelle løsning:'], ...
            ['L{u(t-' t0_str ')} = (e^(-s·' t0_str ')/s) · u(' t0_str ') + (1/s) · u(-' t0_str ')']);
        
        % Skab en stykkevis defineret symbolsk løsning
        F = piecewise(t0 > 0, exp(-s*t0)/s, t0 == 0, 1/s, 1/s);
        
        forklaringsOutput = ElektroMat.Forklaringssystem.tilfoejTrin(forklaringsOutput, 5, ...
            'Tolkning af den symbolske løsning', ...
            ['Løsningen afhænger af værdien af ' t0_str ':'], ...
            ['- Hvis ' t0_str ' > 0: L{u(t-' t0_str ')} = e^(-s·' t0_str ')/s' ...
             '\n- Hvis ' t0_str ' = 0: L{u(t)} = 1/s' ...
             '\n- Hvis ' t0_str ' < 0: L{u(t-' t0_str ')} = 1/s']);
    else
    ...
    % Sikr at F_str kan håndtere både symbolske og numeriske værdier
    try
