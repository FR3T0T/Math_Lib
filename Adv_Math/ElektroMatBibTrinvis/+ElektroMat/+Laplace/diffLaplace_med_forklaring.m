function [G, forklaringsOutput] = diffLaplace_med_forklaring(F_s, s, n)
    ...
    % Start forklaring
    forklaringsOutput = ElektroMatBibTrinvis.ElektroMat.Forklaringssystem.startForklaring(['Differentiering af Laplacetransformation (t^' num2str(n) 'f(t))']);
    ...
    for i = 1:n
        if i == 1
            G = -diff(F_s, s);
            forklaringsOutput = ElektroMatBibTrinvis.ElektroMat.Forklaringssystem.tilfoejTrin(forklaringsOutput, 2+i, ...
